...
namespace HospitalityDoors
{
    public abstract class Gizmo_ModifyNumber<T> : Gizmo
    {
        ...
        private void DrawButtons(Rect rect)
        {
            ...
            // Create simple colored buttons since we don't have the original textures
            var originalColor = GUI.color;
            GUI.color = ButtonColor;

            if (Widgets.ButtonText(middleSetting, "Auto"))
            {
                ButtonCenter();
            }

            if (Widgets.ButtonText(upperSetting, "+"))
            {
                ButtonUp();
            }

            if (Widgets.ButtonText(lowerSettings, "-"))
            {
                ButtonDown();
            }
